...
export default {
  data () {
    return {
      ...
      gojuon: JSON.parse(localStorage.getItem('both')),
      cursor: 0,
      userAnswer: '',
      helpAnswer: '',
  ...
  watch: {
    userAnswer (newAnswer) {
      if (this.currentExam[this.cursor][this.gType === 'hiragana' ? 1 : 0] === newAnswer.toLowerCase()) {
        this.cursor += 1
  ...
  methods: {
    ...
    generateExam () {
      return this.shuffle(this.gojuon)
    }
  },
  directives: {
    focus: {
